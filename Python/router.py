...
# 最近一次路由命中的快照；路由设置页用 GET /router/state 读它。
# 单条容量（占位实现，未来路由 v2 / 编排时整片重写）。
_LAST_DECISION: RoutingDecision | None = None
...
def _record(decision: RoutingDecision) -> RoutingDecision:
...
def preview(
    ...
    *,
    ...
    """只读路由：和 route 同样的瀑布逻辑，但**不写**全局 _LAST_DECISION。
    模型管理页"via X"探测、UI 预览类调用都走这个，避免污染 RouterPage 的最近命中。
    """
    # 1. 显式
    if explicit_model:
...
def route(
    ...
    *,
    ...
    """真正聊天链路用：preview 决策 + 写全局 _LAST_DECISION。"""
    return _record(preview(message, registry, explicit_model=explicit_model))
